                        ...
                        virkshop._internal.deadlines.beforeShellScripts.push(
                            Promise.all(Object.keys(pluginOutput?.commands||{}).map(async (commandName)=>{
                                ...
                                await FileSystem.write({
                                    ...
                                    path: executablePath,
                                    data: [
                                        `#!/usr/bin/env -S ${posixShellEscape(absolutePathToDeno)} run --no-lock --no-config -q --allow-all`,
                                    ...
                                    async (eventName)=>{
                                        try {
                                            ...
                                            for (const [deadlineName, deadlineHook] of Object.entries(deadlineResponses)) {
                                                const apiArg = virkshop._internal.createApi({
                                                    source: pluginPath,
                                                    eventName,
                                                    deadlineName,
                                                })
                                                if (!(virkshop._internal.deadlines[deadlineName] instanceof Array)) {
                                                    ...
                                                    virkshop._internal.deadlines[deadlineName].push(
                                                        deadlineHook.apply(pluginOutput, apiArg)
                                                    )
                                                }
                                            }
                                        } catch (error) {
            ...
            _stages: {
                ...
                async phase0() {
                    ...
                    virkshop._internal.deadlines.beforeSetup.push(
                        glob(autogeneratedPattern, {startPath: virkshop.pathTo.events, }).then((args)=>{
                            return FileSystem.remove(args)
                        })
                    )
                            ...
                            if (eachPath.match(/\.deno\.js$/)) {
                                virkshop._internal.shellSetupPriorities.push(
                                    [
                                        ...
                                        `deno run -q -A --no-lock --no-config ${shellApi.escapeShellArgument(eachPath)}`,
                                    ]
                                )
...
export const shellApi = Object.defineProperties(
    {
        ...
        shellProfileString: `
            ...
            if ! [ "$VIRKSHOP_DEBUG" = "0" ]; then
                deno eval --no-lock --no-config 'console.log(\`    [\${(new Date()).getTime()-Deno.env.get("_shell_start_time")}ms nix-shell]\`)'
            fi
